...
import { create, find, countDocuments, findById, findByIdAndUpdate } from '../models/DiagnosticCenter.js';
import { findById as _findById, findByIdAndUpdate as _findByIdAndUpdate } from '../models/User.js';
...
const createCenter = async (req, res) => {
  try {
    ...
    if (adminId) {
      const admin = await _findById(adminId);
    ...
    if (adminId) {
      await _findByIdAndUpdate(adminId, { diagnosticCenterId: center._id });
    }
...
const getAllCenters = async (req, res) => {
  try {
    ...
    const centers = await find(query)
...
const getCenterById = async (req, res) => {
  try {
    const center = await findById(req.params.id)
      .populate('adminId', 'name email phone');
...
const updateCenter = async (req, res) => {
  try {
    ...
    const center = await findByIdAndUpdate(
      centerId,
      updates,
